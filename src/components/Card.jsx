...
import { addToFavorite, deletePic } from '../redux/gallerySlice'
...
const Card = ({ cardItem }) => {
    ...
    const likeThisCat = (cardItem)=>{
        setLike(!like)
        dispatch(addToFavorite(cardItem))
    }
    return (
        ...
        <div className="card" >
            ...
            <button  className={like ? "like-button-liked":"like-button" } onClick={()=>likeThisCat(cardItem)}>
               {like?"Deslike":"Like"} 
            </button>
            </div>
        </div>
